...
<xsl:stylesheet
	...
	<!-- XSLT stylesheet to pretty print grammar.xml Copyright (C) 2008 Marcin 
		Miłkowski * This library is free software; you can redistribute it and/or 
		* modify it under the terms of the GNU Lesser General Public * License as 
		...
		St, Fifth Floor, Boston, MA 02110-1301 * USA usage: java -jar saxon8.jar 
		grammar.xml print.xsl This version doesn't work in Firefox, unfortunately... -->
	...
	<xsl:template match="//rule/example[@type='incorrect']">
		<li>
			...
			<xsl:if test="@correction !=''">
				<xsl:choose>
					<xsl:when
						test="not(contains(@correction, '|')) and not(contains(../message/text()[1], '\')) and count(../message/text()) &lt; 3">
					...
					<xsl:otherwise>
						<!-- Remaining problem: replace \1 in message text with pattern/token[1] 
							<xsl:choose> <xsl:when test="//rules[@lang='pl']">Poprawnie: </xsl:when> 
							<xsl:when test="//rules[@lang='en']">Correctly: </xsl:when> <xsl:when test="//rules[@lang='de']">Korrekt: 
							</xsl:when> <xsl:when test="//rules[@lang='fr']">Correctement : </xsl:when> 
							<xsl:when test="//rules[@lang='nl']">Correct: </xsl:when> <xsl:when test="//rules[@lang='es']">Correctamente: 
							</xsl:when> </xsl:choose> <strong style="color: #339900;"> <xsl:value-of 
	...
	<xsl:template
		match="//rule/example[@type='incorrect']/text()">
		<xsl:copy-of select="." />
	</xsl:template>

	<xsl:template
		match="//rule/example[@type='incorrect']/marker">
