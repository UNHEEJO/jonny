...
public class JLanguageTool {
  ...
  public List<RuleMatch> check(AnnotatedText annotatedText, boolean tokenizeText, ParagraphHandling paraMode, RuleMatchListener listener) throws IOException {
    ...
    System.out.println("STK Checker에 들어갈 문장 = "+analyzedSentences);
    AnalyzedTokenReadings[] temp_tokens = analyzedSentences.get(0).getTokensWithoutWhitespace();
    for(AnalyzedTokenReadings testing_token : temp_tokens) {
    	System.out.println("Token : " + testing_token.getToken());
    	
    	for(AnalyzedToken analyzedToken : testing_token.getReadings()) {
